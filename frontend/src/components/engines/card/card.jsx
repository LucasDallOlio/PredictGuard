...
export function CardMotor({ motor, onDelete, onClick }) {
      ...
      <Card className="flex w-full max-w-6xl mx-auto flex-col md:flex-row overflow-hidden py-0">
        ...
        <CardContent className="flex flex-col justify-between p-6 w-full">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Setor: {motor.setor}</p>
            <p className="text-muted-foreground text-sm mb-4">Código: {motor.codigo}</p>
            ...
            <div className="flex gap-4 flex-wrap">
              ...
              <div className={`flex items-center gap-2 px-3 py-1 rounded-lg text-sm ${motor.vibracao === "Normal" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
                <Activity size={16} />
                {motor.vibracao}
              </div>
            </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button
              variant="destructive"
              // Ao invés de deletar direto, abre o modal
