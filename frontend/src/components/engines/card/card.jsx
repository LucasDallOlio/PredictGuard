...
import {
  Trash2, Eye, AlertTriangle, CheckCircle2,
...
function LinhaMotor({ motor, onClick, onDelete }) {
  ...
  const statusOpLabel =
    statusOp === "ativa" ? "Ativa"
    : statusOp === "manutencao" ? "Manutenção"
    : "Parada";
      ...
      <tr className="group hover:bg-sky-50/50 dark:hover:bg-sky-950/20 transition-colors">
        ...
        <td className="px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-gray-800 dark:text-zinc-100 leading-tight">{motor.nome}</p>
              <p className="text-xs text-gray-400 dark:text-zinc-500">{motor.cod_registro}</p>
            </div>
          </div>
        </td>
        ...
        {/* Status operacional */}
        <td className="px-5 py-4">
          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${statusOpStyle}`}>
            {statusOpLabel}
          </span>
        </td>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm overflow-hidden">
        {motores.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-gray-400 dark:text-zinc-500">Nenhum motor para exibir.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-zinc-800">
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wider">Motor</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wider">Setor</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wider">Operacional</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wider">Saúde</th>
